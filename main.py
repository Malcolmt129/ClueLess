...
def handle_server_message(message_object):
    ...
    if isinstance(message_object, ErrorMessage):
        ...
        # === NEW CODE START ===
        # First player sends custom names to server
        if user_id == 0:  # First player is host
            characters = [running_game.CHARACTERS[char] for char in Characters]
            weapons = [running_game.WEAPONS[weapon] for weapon in Weapons]
            rooms = [running_game.ROOMS[room] for room in Rooms]

            client.send_message(StateUpdateMessage(
                user_id=user_id,
                updates={
        ...
        # === NEW CODE START ===
        # Handle custom names update
        if 'custom_characters' in message_object.updates:
            # Update character mappings
            for i, enum_char in enumerate(Characters):
                custom_name = message_object.updates['custom_characters'][i]
                running_game.CHARACTERS[enum_char] = custom_name
                running_game.CHARACTERS_REVERSE[custom_name] = enum_char

            # Update weapon mappings
            for i, enum_weapon in enumerate(Weapons):
                custom_name = message_object.updates['custom_weapons'][i]
                running_game.WEAPONS[enum_weapon] = custom_name
                running_game.WEAPONS_REVERSE[custom_name] = enum_weapon

            # Update room mappings
            for i, enum_room in enumerate(Rooms):
                custom_name = message_object.updates['custom_rooms'][i]
                running_game.ROOMS[enum_room] = custom_name
                running_game.ROOMS_REVERSE[custom_name] = enum_room
        # === NEW CODE END ===
